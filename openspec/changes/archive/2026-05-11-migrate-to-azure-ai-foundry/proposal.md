...
- `src/Backlog2Spec.Cli/Kernel/KernelFactory.cs` — add endpoint-type branching
- `src/Backlog2Spec.Cli/Program.cs` — read renamed secrets and pass endpoint type to factory
- `README.md` — updated setup instructions for Azure AI Foundry (and kept Azure OpenAI as secondary option)
- `tests/Backlog2Spec.Tests/` — no changes required (kernel is not directly tested; agents are tested via mocks)
