...
- [x] 1.1 Delete `src/Backlog2Spec.Cli/Ado/WikiPageDto.cs`
- [x] 1.2 Delete `src/Backlog2Spec.Cli/Agents/IWikiContextAgent.cs`
- [x] 1.3 Delete `src/Backlog2Spec.Cli/Agents/WikiContextAgent.cs`
- [x] 1.4 Delete `src/Backlog2Spec.Cli/Agents/MockWikiContextAgent.cs`

## 2. Add Codebase Context DTOs and Interface

- [x] 2.1 Create `src/Backlog2Spec.Cli/Ado/CodeFileDto.cs` with properties `Path`, `FileName`, `Content` (all string)
- [x] 2.2 Create `src/Backlog2Spec.Cli/Agents/ICodebaseContextAgent.cs` with method `FetchRelevantFilesAsync(WorkItemDto, AgentConfig, CancellationToken)`

## 3. Implement CodebaseContextAgent

- [x] 3.1 Create `src/Backlog2Spec.Cli/Agents/CodebaseContextAgent.cs` — constructor accepts `string pat` and `ILogger<CodebaseContextAgent>`
...
- [x] 4.1 Create `src/Backlog2Spec.Cli/Agents/MockCodebaseContextAgent.cs` implementing `ICodebaseContextAgent`
...
- [x] 5.1 In `src/Backlog2Spec.Cli/Config/AgentConfig.cs`: remove `WikiName` from `AdoConfig`, add `RepoName` (string?) and `Branch` (string?)
...
- [x] 10.1 Run `dotnet build src/Backlog2Spec.Cli` — zero errors
- [x] 10.2 Run `dotnet backlog-2-spec spec 42 --mock` — pipeline completes successfully end-to-end
- [x] 10.3 Run `dotnet test tests/Backlog2Spec.Tests` — all tests pass
