...
- [x] 1.1 Create solution: `dotnet new sln -n Backlog2Spec` inside `Backlog2Spec/`
- [x] 1.2 Create CLI project: `dotnet new console -n Backlog2Spec.Cli -o src/Backlog2Spec.Cli --framework net8.0`
- [x] 1.3 Create test project: `dotnet new xunit -n Backlog2Spec.Tests -o tests/Backlog2Spec.Tests --framework net8.0`
- [x] 1.4 Add both projects to solution: `dotnet sln add src/Backlog2Spec.Cli tests/Backlog2Spec.Tests`
- [x] 1.5 Add project reference from Tests to Cli: `dotnet add tests/Backlog2Spec.Tests reference src/Backlog2Spec.Cli`
- [x] 1.6 Configure `Backlog2Spec.Cli.csproj`: set `<PackAsTool>true</PackAsTool>`, `<ToolCommandName>backlog-2-spec</ToolCommandName>`, `<Version>0.1.0</Version>`, `<UserSecretsId>` (generate a GUID)
- [x] 1.7 Create all subdirectory stubs: `Commands/`, `Agents/`, `Ado/`, `Models/`, `Config/`, `Kernel/`, `Output/`, `Prompts/`

## 2. NuGet Dependencies

- [x] 2.1 Add `Microsoft.SemanticKernel` to `Backlog2Spec.Cli`
- [x] 2.2 Add `Microsoft.TeamFoundationServer.Client` (Azure DevOps SDK) to `Backlog2Spec.Cli`
- [x] 2.3 Add `System.CommandLine` (2.0.0-beta4.x) to `Backlog2Spec.Cli`
- [x] 2.4 Add `Spectre.Console` to `Backlog2Spec.Cli`
- [x] 2.5 Add `HtmlAgilityPack` to `Backlog2Spec.Cli`
- [x] 2.6 Add `Microsoft.Extensions.Hosting`, `Microsoft.Extensions.Logging`, `Microsoft.Extensions.DependencyInjection` to `Backlog2Spec.Cli`
- [x] 2.7 Run `dotnet restore` and verify no package conflicts

## 3. Configuration & Secrets

- [x] 3.1 Create `backlog-2-spec.json` at `Backlog2Spec/` root with the specified schema (use placeholder org/project values)
...
- [x] 3.5 Run `dotnet user-secrets init --project src/Backlog2Spec.Cli`
...
- [x] 13.1 Create `Backlog2Spec.Tests/AdoClientTests.cs` — empty test class with one `[Fact]` placeholder
- [x] 13.2 Create `Backlog2Spec.Tests/EnrichmentAgentTests.cs` — empty test class with one `[Fact]` placeholder
- [x] 13.3 Create `Backlog2Spec.Tests/SpecGeneratorAgentTests.cs` — empty test class with one `[Fact]` placeholder
- [x] 13.4 Create `Backlog2Spec.Tests/ConfigLoaderTests.cs` — empty test class with one `[Fact]` placeholder
- [x] 13.5 Create `Backlog2Spec.Tests/OutputRendererTests.cs` — empty test class with one `[Fact]` placeholder
...
- [x] 14.3 Install tool locally: `dotnet tool install --local Backlog2Spec.Cli` (or pack and install)
