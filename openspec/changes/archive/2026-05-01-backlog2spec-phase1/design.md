...
**Decision**: Single project (`Backlog2Spec.Cli`) with subdirectories enforcing layer roles by convention (Agents/, Ado/, Output/, etc.) rather than separate class library projects.

**Rationale**: For Phase 1 scope, a multi-project solution would add friction with no benefit — the interface boundaries already prevent layer leakage. Phase 2 can extract `Backlog2Spec.Core` if the solution grows. Keeping one project speeds up bootstrap and tooling (user-secrets, dotnet tool packaging).

**Alternative considered**: Separate `Backlog2Spec.Core` and `Backlog2Spec.Cli` projects. Rejected — over-engineering for Phase 1; interfaces achieve the same decoupling contract.
...
- Phase 2 will need a `plan` command — should `KernelFactory` become a shared service in an `Backlog2Spec.Core` library at that point? (Yes — defer to Phase 2)
