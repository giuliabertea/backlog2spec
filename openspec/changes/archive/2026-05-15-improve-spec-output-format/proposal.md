...
- `src/Backlog2Spec.Cli/Models/GeneratedSpec.cs` — field renames/additions
- `src/Backlog2Spec.Cli/Prompts/spec.txt` — new JSON schema, rules, and example
- `src/Backlog2Spec.Cli/Agents/SpecGeneratorAgent.cs` — template variable for `affectedComponents` → `filesToChange` in output schema
- `src/Backlog2Spec.Cli/Output/OutputRenderer.cs` — updated section rendering for all output modes (console, markdown, hierarchy)
